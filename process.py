...
class ownprocess:
    def __init__(self, args, namespace=None, cwd=None, print_f=print,
                 additionalargs=None):
    ...
    def run(self, exceptionok=False):
        process = Popen(self.args, cwd=self.cwd, stdin=PIPE, stdout=PIPE,
                        stderr=PIPE)
        self.stdout, self.stderr = process.communicate()
        if not exceptionok:
            assert process.returncode == 0, (self.stdout, self.stderr)
    ...
    def check_stderr(self):
        ...
        with self.lock:
            if not self.waskilled:
                poll = self.p.poll()
                assert poll is not None, (self.waskilled, poll)
                assert poll == 0, (self.waskilled, poll)

    def run_bg(self):
        self.p = Popen(self.args, cwd=self.cwd, stdin=PIPE, stdout=PIPE,
                       stderr=PIPE)
        self.threads = [Thread(target=self.check_stderr),
                        Thread(target=self.check_stdout)]
...
def run(args, namespace=None, cwd=None, exceptionok=False, bg=False,
        additionalargs=None):
    op = ownprocess(args, namespace, cwd, print_f=logging.info,
